...
const bgfx::ShaderHandle loadShaderFile(const char* filename) {
    ...
    if (!file.is_open())
        throw FOLK_RUNTIME_ERROR("Couldn't open file " + std::string(filename));
    ...
    if (!bgfx::isValid(handle))
        throw FOLK_RUNTIME_ERROR("Shader load failed");
...
{       
    ...
    if (!bgfx::isValid(program))
        throw FOLK_RUNTIME_ERROR("Shader program linking failed");
